-module(gl_sampler_parameter_i_readback_test).
-include_lib("eunit/include/eunit.hrl").

sampler_parameter_i_readback_test() ->
    gl_test_context:setup_context(),

    {ok, [Sampler]} = gl:gen_samplers(1),

    ok = gl:sampler_parameter_i(i, Sampler, texture_wrap_s, [16#812F]),
    {ok, [16#812F]} = gl:get_sampler_parameter_i(i, Sampler, texture_wrap_s, 1),

    ok = gl:sampler_parameter_i(ui, Sampler, texture_wrap_t, [16#812F]),
    {ok, [16#812F]} =
        gl:get_sampler_parameter_i(ui, Sampler, texture_wrap_t, 1),
