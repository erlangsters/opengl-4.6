-module(gl_multi_draw_offset_test).
-include_lib("eunit/include/eunit.hrl").

multi_draw_offset_test() ->
    ...
    ok = gl:buffer_data(element_array_buffer, <<0, 0>>, static_draw),
    ...
    ok = assert_draw_green(fun() -> gl:multi_draw_arrays(points, [{0, 1}, {0, 1}]) end),
    ok = assert_draw_green(fun() -> gl:multi_draw_elements(points, unsigned_byte, [{1, 0}, {1, 1}]) end),
    ok =
        assert_draw_green(
            fun() ->
                gl:multi_draw_elements_base_vertex(points, unsigned_byte, [{1, 0, 0}, {1, 1, 0}])
            end
        ),
