-module(gl_double_matrix_uniform_rendered_pixel_test).
-include_lib("eunit/include/eunit.hrl").

double_matrix_uniform_rendered_pixel_test() ->
    ...
    ok = gl:uniform_matrix(d, Location, green_matrix()),
    ...
    ok = gl:program_uniform_matrix(d, Program, Location, blue_matrix()),
    ok = gl:use_program(Program),
    ok = gl:draw_arrays(points, 0, 1),
    {ok, <<0, 0, 255, 255>>} =
        gl:read_pixels(0, 0, 1, 1, rgba, unsigned_byte, 4),
...
fragment_source() ->
    <<"#version 400\n"
      "uniform dmat4 u_matrix;\n"
      "out vec4 frag_color;\n"
      "void main() { frag_color = vec4(u_matrix[0]); }\n">>.
